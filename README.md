...
| 源 | 车源数 | 源站最新挂牌 | 状态 |
|---|---|---|---|
| 21rv | 3224 | 2026-04-23 | ⚠ 上游停更 147 天（接口本身正常） |
| rv28 | 394 | 每日更新 | 正常 |
| cn2rv | 236 | 偶有新增 | 正常 |

判断口径：`crawl_21rv.py --status` 会拉取列表首页，取源站 `created_at` 的最大值；
超过 `RV_STALE_DAYS`（默认 14 天）没有新挂牌，页面与日志就会打上"停更"标记。
...
| 数据源健康 | 页头显示各源最后挂牌时间；某源长期停更时给出提示并说明"不是抓取故障"，可关闭（按天记忆） |
...
6. **`posted_at`（源站挂牌时间）** —— 新增字段与自动迁移；`export_data.py` 同步导出 `window.RV_SOURCES` 新鲜度数据。
